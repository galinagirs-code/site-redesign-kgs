...
const UdlinennayaStrela = () => {
  ...
  const productSchema = {
    ...
    "description": "Удлинённая стрела для увеличения глубины погружения свай. Изготавливается под конкретную модель экскаватора.",
    "brand": { "@type": "Brand", "name": "KGS" },
    "category": "Дополнительное оборудование",
    "manufacturer": { "@type": "Organization", "name": "КоперГруппСервис" }
  };

  const galleryImages = ["https://cdn.poehali.dev/files/8d5ff46f-c1e3-455c-b4bb-8e35f5ef25aa.png","https://cdn.poehali.dev/files/8d5ff46f-c1e3-455c-b4bb-8e35f5ef25aa.png","https://cdn.poehali.dev/files/8d5ff46f-c1e3-455c-b4bb-8e35f5ef25aa.png","https://cdn.poehali.dev/files/8d5ff46f-c1e3-455c-b4bb-8e35f5ef25aa.png"];

  const questions = [{ question: "Грузоподъемность базовой машины?" },{ question: "Что планируете погружать?" },{ question: "Глубина погружения?" },{ question: "Диаметр трубы?" },{ question: "Какие требуются зажимы?", options: ["Труба", "Шпунт", "Сваи"] },{ question: "Какие будут грунты?" },{ question: "Требуется ли шефмонтаж?", options: ["Да", "Нет"] },{ question: "Когда планируется объект?" },{ question: "В какой город осуществлять доставку?" }];

  return (
    <div className="min-h-screen">
      <SEO title="Удлинённая стрела для экскаватора | KGS" description="Удлинённая стрела для экскаватора позволяет увеличить глубину погружения свай. Изготовление под конкретную модель базовой машины." keywords="удлиненная стрела экскаватора купить, стрела для вибропогружателя, удлинитель стрелы экскаватора" canonical="https://kgs-ural.ru/catalog/udlinennaya-strela" ogTitle="Удлинённая стрела для экскаватора - увеличение глубины погружения" ogDescription="Удлинённая стрела под вашу модель экскаватора. Увеличение рабочей зоны и глубины погружения свай." />
      ...
      <main className="pt-16">
        <div className="container mx-auto px-4 py-8">
          <Breadcrumbs items={[{ label: "Главная", href: "/" }, { label: "Каталог", href: "/catalog" }, { label: "Вибропогружатели экскаваторные", href: "/catalog#excavator-vibro-drivers" }, { label: "Удлинённая стрела для экскаватора" }]} />

          <div className="grid lg:grid-cols-2 gap-8 mt-8">
            <div>
              <div className="relative aspect-square rounded-lg overflow-hidden bg-gray-100 mb-4"><OptimizedImage src={selectedImage || galleryImages[0]} alt="Удлинённая стрела для экскаватора" className="w-full h-full object-contain" /></div>
            ...
            <div>
              <Badge className="mb-4">Дополнительное оборудование</Badge>
              <h1 className="text-3xl font-bold mb-4">Удлинённая стрела для экскаватора</h1>
              
              <div className="prose max-w-none mb-6">
                <h2 className="text-xl font-semibold mb-3">Удлинённая стрела для экскаватора</h2>
                <p className="text-gray-600 mb-4">Удлинённая стрела позволяет увеличить глубину погружения свай и расширяет рабочую зону экскаватора. Изготавливается под конкретную модель базовой машины с учетом требуемой длины и грузоподъемности.</p>
              </div>
          ...
          <Card className="mt-12"><CardContent className="p-8"><h2 className="text-2xl font-bold mb-6">Получить консультацию</h2><EquipmentForm equipmentType="Удлинённая стрела для экскаватора" questions={questions} /></CardContent></Card>
        </div>
      </main>
